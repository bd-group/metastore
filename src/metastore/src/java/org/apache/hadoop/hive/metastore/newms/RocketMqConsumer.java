...
public class RocketMqConsumer {
  ...
  private RocketMqConsumer(String nameSrvAddr, String topic, String group) throws MetaException, IOException {
    RocketMqConsumer.topic = topic;
    RocketMqConsumer.nameSrvAddr = nameSrvAddr;
    RocketMqConsumer.group = group;
    consumer = new DefaultMQPushConsumer(group);
    mp = new MsgProcessing();
  }

  public static RocketMqConsumer getInstance(String nameSrvAddr, String topic, String group) throws MetaException, IOException {
    if (instance == null) {
      instance = new RocketMqConsumer(nameSrvAddr, topic, group);
    }
    return instance;
  }
  ...
  public void consume() throws MQClientException{
    ...
    /*消费者实例的名字，一个消费者组子啊可以有多个实例，实例的需要名字不同*/
    ...
    consumer.start();
    LOG.debug("libing,debug:rmqConsumer Started.");
  }
}
